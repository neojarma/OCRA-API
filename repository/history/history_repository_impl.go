...
func (repository *WatchLaterRepositoryImpl) IsRecordExist(req *entity.Watch_Laters) bool {
	return repository.Db.Where("video_id = ? ", req.VideoId).Where("channel_id = ?", req.ChannelId).Find(req).RowsAffected == 1
}
